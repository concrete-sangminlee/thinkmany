...
## 실험 추적 — Git을 넘어서는 박사 연구의 기록 시스템

Git은 **코드·텍스트의 버전 관리**에는 완벽하지만 **실험의 추적**에는 부족하다. ML/AI·공학 박사의 실험은 수십-수백 번의 파라미터 조합·모델·데이터셋의 조합으로 구성된다. "어떤 하이퍼파라미터로 어떤 결과가 나왔는가"를 Git commit만으로 관리하면 혼란. 2020년 이후 **Experiment Tracking** 도구(W&B·MLflow·Neptune·DVC·ClearML)가 표준이 됐다. 이 섹션은 Git을 넘어선 박사의 실험 기록 시스템을 다룬다.

<div class="highlight-box info">
<span class="highlight-box-icon">ℹ️</span>
<div class="highlight-box-content">
<p><strong>왜 Git만으로 부족한가</strong></p>
<p>박사 실험의 질문들: "어제 lr=1e-3으로 돌렸을 때 정확도가 얼마였지?" "3개월 전 가장 좋은 모델의 config는?" "이 실험 결과를 다시 돌리려면 어떤 data·code·environment?" Git이 답할 수 있는 건 코드 변경 이력뿐. 하이퍼파라미터·데이터 버전·메트릭·모델 가중치·GPU 상태·랜덤 시드 등은 Git 밖. Experiment Tracking이 이 공백을 메운다.</p>
</div>
</div>
...
| 도구 | 강점 | 가격 | 적합 |
|---|---|---|---|
| Weights & Biases (W&B) | UI 가장 좋음, ML 커뮤니티 표준 | 학생 무료 (Academic) | ML·DL 박사, 특히 논문 발표 |
| MLflow | 오픈 소스, 자체 호스팅 가능 | 무료 | 프라이버시 중시, 산업 연구 |
| Neptune | UI 깔끔, 협업 기능 | 학생 무료 (Academic) | 팀 프로젝트 |
| DVC (Data Version Control) | 데이터·모델의 Git-like | 무료 | 재현성 중시, 큰 데이터 |
| TensorBoard | PyTorch·TF 기본 통합 | 무료 | 간단한 추적, 학습 시작점 |
| ClearML | Enterprise 기능 많음 | 학생 무료 | 큰 프로젝트 |
| Aim | 오픈 소스, 로컬 UI | 무료 | 심플함 선호 |
| Comet | 친근한 UI | 학생 무료 | ML 초보 |

**박사 권장**: 
- **ML/AI 박사**: W&B (학생 무료 + 커뮤니티 표준)
- **프라이버시 중요**: MLflow (자체 호스팅)
- **큰 데이터셋**: DVC 조합 (코드는 Git + 데이터는 DVC)

### W&B를 박사 워크플로우에 통합 (예시)

W&B (가장 널리 쓰이는 도구) 사용 예:
...
# 실험 시작
wandb.init(
    project="phd-thesis-pinn",
    name="run-2025-11-15-lr-sweep",
    config={
        "learning_rate": 1e-3,
        "batch_size": 32,
        "architecture": "PINN-5-layer",
        "dataset": "burgers-equation-v3"
    }
)
...
# 결과 저장
wandb.log({"final_l2_error": compute_error()})
wandb.save("model.pt")

wandb.finish()
```

이 10여 줄의 코드로 모든 실험이 자동 기록되고 웹 대시보드에서 비교 가능.

### 박사 실험 추적의 5가지 원칙

**원칙 1: 실험 시작 시 무조건 기록**.
"이건 간단한 실험이니 기록 안 해도 돼"의 유혹 피함. 3개월 후 돌아보면 기록 안 한 실험을 기억할 수 없음. 매 실험에 1분의 기록 오버헤드.

**원칙 2: 메타데이터 충분히**.
하이퍼파라미터뿐 아니라: 코드 commit hash, 데이터 버전, GPU 종류, Python·라이브러리 버전, 랜덤 시드, 실행 시간. 재현의 모든 요소.

**원칙 3: 메트릭의 충분한 세분화**.
"정확도 95%"뿐 아니라 클래스별 정확도, confusion matrix, 학습 곡선, 샘플 예측. 나중에 문제 파악에 필요.

**원칙 4: 아티팩트 저장**.
모델 가중치, 학습 그래프, 샘플 시각화를 W&B·MLflow에 업로드. 3개월 후 코드 없이도 결과를 다시 볼 수 있음.

**원칙 5: 실험 태그와 노트**.
각 실험에 태그 (experiment, production, debug 등)와 한 줄 노트 ("lr 스윕", "새 데이터셋 시도"). 나중에 필터링·검색이 쉬워짐.

### 데이터 버전 관리 — DVC

**왜 필요한가**: 박사 데이터셋이 수 GB-수 TB. Git에 올릴 수 없음. 그러나 데이터 버전 없이는 재현성 불가.

**DVC 개념**:
- 데이터 파일을 Git처럼 관리
- 실제 데이터는 S3·GCS·Google Drive 등 원격 저장
- Git에는 데이터의 메타데이터·해시만

**박사 활용**:
```bash
# 데이터 추적 시작
dvc add data/raw/experiments.h5

# 버전 기록
git add data/raw/experiments.h5.dvc
git commit -m "data: add experiment dataset v1"

# 원격 저장
dvc push
```

**장점**: 코드의 특정 commit과 데이터의 특정 버전이 정확히 연결. 협력자가 `git clone` + `dvc pull`로 동일한 상태 재현.

**단점**: 학습 곡선 있음. 작은 데이터셋(< 1GB)에는 과할 수 있음. MLflow·W&B와 조합 시 복잡성.

### Jupyter Notebook + 실험 추적

Jupyter Notebook에서 실험 추적 통합:
- 노트북 상단에 `wandb.init()`
- 각 실험 셀의 결과를 `wandb.log()`
- 노트북 자체를 아티팩트로 저장 (`wandb.save('analysis.ipynb')`)
- 노트북의 출력·그래프도 자동 캡처 가능

ch16 기존 섹션의 "Jupyter Notebook과 Git" 문제를 W&B로 완화. Notebook은 탐색에, W&B는 실험 기록에.

### 실험 추적의 흔한 실수 5가지

**실수 1: 뒤늦은 시작**.
박사 3-4년차에야 W&B 시작. 이전 실험은 기록 없음. 박사 논문 작성 시 과거 실험 결과 찾기 어려움.

**대응**: 박사 1-2년차에 도구 선택·설정. 작은 실험도 기록.

**실수 2: 과도한 기록**.
모든 것을 기록하려다 실험 실행 자체가 느려짐. 매 스텝의 모든 텐서를 저장.

**대응**: 핵심 메트릭만. 대용량 데이터는 주기적으로 (매 N 스텝).

**실수 3: 팀 공유 미설정**.
본인만의 로컬 계정·프로젝트. 공동 연구자가 결과를 볼 수 없음.

**대응**: 연구실 단위 team 계정. 논문 공저자 초대.

**실수 4: 도구 변경의 비용**.
W&B에서 MLflow로 옮기려면 과거 실험 전환 어려움. 한 번 선택하면 박사 과정 내내 유지 권장.

**대응**: 처음 3개월에 여러 도구 시험. 이후 하나로 확정.

**실수 5: 실험 추적 ≠ 재현성**.
W&B에 기록했다고 자동으로 재현 가능 아님. 코드·데이터·환경도 함께 관리해야 함 (ch38 참조).

**대응**: 실험 추적 + Git + DVC + Docker의 조합.

### 박사 논문과의 연결

박사 논문의 실험 섹션은 수백-수천 실험의 축적. 실험 추적이 없으면 논문 작성이 악몽.

**박사 논문 작성 시 활용**:
- W&B에서 실험을 필터·정렬해 표·그림 생성
- 메트릭의 공식 리포트 자동 생성
- 논문에 포함된 실험의 reproducibility statement
- 리뷰어 질문에 W&B 링크로 즉시 증거 제시

**실전 팁**: 박사 논문 작성 6개월 전에 **W&B의 프로젝트 구조 정리**. 실험을 박사 논문의 챕터·섹션에 매핑. 이후 논문 작성이 훨씬 쉬움.

### 한국 박사의 도구 선택

**비용 고려**:
- W&B·Neptune·ClearML의 학생 무료 플랜: 대부분의 박사 용도에 충분
- 학교·랩이 Enterprise 라이선스 보유: 확인하고 활용
- MLflow·DVC·Aim: 무료 오픈 소스, 자체 호스팅

**프라이버시 고려**:
- 산업 협력 과제: 데이터·모델이 외부 클라우드에 올라가면 안 되는 경우. MLflow 자체 호스팅 또는 랩 서버.
- 공공 연구: W&B 같은 SaaS 서비스 무난.

**한국어 지원**:
- W&B·MLflow 등은 UI가 영어. 한국어 지원 제한적.
- 문서·튜토리얼은 영어 원문 + 한국어 블로그 커뮤니티 참고.

### 박사 단계별 실험 추적 로드맵

**박사 1-2년차**: 도구 1개 선택·설정. 모든 실험 기록 시작.

**박사 3-4년차**: DVC로 데이터 버전 추가. 팀 공유 설정.

**박사 5-7년차**: 박사 논문과 연결된 체계적 프로젝트 구조. 리포트 생성.

**박사 후**: 새 랩·직장의 도구에 적응. 전환 시 기록 백업.

### 마지막 — 실험 추적은 박사 연구의 두 번째 기록 시스템이다

Git이 **코드의 역사**를 기록하듯 W&B·MLflow는 **실험의 역사**를 기록한다. 두 시스템이 병행되어야 박사 연구가 완전히 추적 가능. 박사 1-2년차에 실험 추적 도구를 설정하고 모든 실험을 기록하는 습관을 만들면 박사 5-7년의 수백 실험이 체계적으로 축적된다. 박사 논문 작성 시 이 기록이 3-6개월의 작업을 3-6주로 단축한다.

> Git이 코드의 버전 관리라면 W&B·MLflow는 실험의 버전 관리다. 박사 1-2년차에 도구를 선택·설정하고 모든 실험을 기록하면 박사 5-7년의 축적이 박사 논문의 자산이 된다. Git + 실험 추적 + DVC + 의존성 관리의 4중 시스템이 박사 연구의 완전한 재현성을 보장한다.

---

## 박사의 백업 전략 — "3-2-1 규칙과 재해 복구"

박사 5-7년의 **모든 데이터·코드·논문·노트**가 한순간에 사라질 수 있다. 하드 디스크 고장·랩탑 도난·랜섬웨어·실수로 삭제·화재·홍수. 박사 과정의 공포 스토리 중 **"백업을 안 해서 1년치 실험을 잃었다"**는 너무 흔하다. 박사의 데이터는 **본인의 박사 학위**—이를 보호하는 백업 전략은 **기술이 아니라 의무**다. 잘 설계된 백업은 하루 5분 투자로 **평생의 안심**을 준다.

**3-2-1 백업 규칙 — 업계 표준.**

- **3**: 데이터의 **3개 복사본**.
- **2**: **2개 서로 다른 매체**에 저장.
- **1**: **1개는 오프사이트** (다른 지리적 위치).

이 규칙은 **모든 데이터 손실 시나리오**를 방어한다.

**3-2-1의 실제 적용.**

- **복사본 1 (작업용)**: 본인 랩탑·데스크탑.
- **복사본 2 (로컬 백업)**: 외장 SSD·NAS·연구실 서버.
- **복사본 3 (오프사이트)**: 클라우드 (Dropbox·Google Drive·iCloud·Backblaze).

**이 3가지가 동시에 손실되는 건 거의 불가능**.

**백업 매체의 비교.**

| 매체 | 용량 | 속도 | 내구성 | 비용 |
|------|------|------|--------|------|
| 외장 HDD | 4-20TB | 중 | 3-5년 | 저 |
| 외장 SSD | 1-8TB | 고 | 5-10년 | 중 |
| NAS | 4-40TB | 중 | 5-10년 | 중-고 |
| 클라우드 | 무제한 | 중 | 무제한 | 월/연 |
| LTO 테이프 | 18TB+ | 저 | 30년+ | 고 |

**매체별 장단점**에 따라 조합.

**데이터 손실의 7가지 시나리오.**

- **하드웨어 고장**: HDD 고장 rate 연 2-5%.
- **랩탑 도난·분실**: 카페·공항·학회.
- **실수 삭제**: `rm -rf` 오타.
- **랜섬웨어**: 해외 박사 사례 급증.
- **화재·홍수**: 자연재해.
- **클라우드 서비스 중단**: Provider 폐업.
- **계정 해킹**: 2FA 없는 계정.
...
- **Dropbox**: 파일 동기화·협업. 2TB $120/년.
- **Google Drive·One**: 2TB $100/년. Workspace 포함.
- **iCloud+**: Apple 생태계. 2TB $120/년.
- **OneDrive**: Microsoft. 1TB $70/년 (Office 포함).
- **Backblaze**: Unlimited 백업. $80/년. 무제한.
- **Arq·Carbonite**: 전문 백업 서비스.
- **학교 제공**: 무료인 경우 많음.
...
- **Google Workspace for Education**: 무제한 (감소 중).
- **Microsoft OneDrive for Education**: 1TB+.
- **학교 NAS**: 연구실 공용 공간.
- **고성능 저장소 (HPC)**: 대용량 데이터.
- **ResearchDrive**: 연구용 클라우드.

**학교 서비스는 무료이고 신뢰도 높음**—최우선 활용.

**Git의 백업 역할.**

- **Git은 백업이 아니다**: 버전 관리.
- **GitHub·GitLab·Bitbucket**: 원격 저장소 = 일종의 백업.
- **Private Repo**: 연구 코드.
- **주의**: 데이터·모델은 LFS 또는 별도.

**Git + GitHub**은 코드의 안전.

**대용량 데이터의 백업.**

- **Git LFS**: 100GB까지 (무료), 그 이상 유료.
- **DVC + Cloud Storage**: S3·GCS 백업.
- **Rclone**: 로컬 ↔ 클라우드 동기화.
- **Rsync**: Unix 전통 도구.
- **Duplicati**: 암호화 백업.
...
- **Time Machine (Mac)**: 시간 단위 자동.
- **Windows Backup**: Windows 10·11 내장.
- **rsync cron**: Linux·Unix 스케줄.
- **Dropbox·iCloud**: 실시간 동기화.
- **Backblaze**: 백그라운드 continuous backup.

**자동화 없는 백업은 백업 아님**.

**백업 검증 — "Restore Test".**

- **복구 가능 여부**: 실제로 파일 복구 해보기.
- **주기**: 분기 1회.
- **방법**: 임의 파일 복구.
- **문제 발견**: 파일 손상·접근 불가.
...
- **Versioning**: 이전 버전 보관.
- **Backup**: 손실 방어.
- **둘 다 필요**: 실수로 덮어쓴 파일 복구 + 하드웨어 고장 복구.

**Time Machine, Dropbox 등은 Versioning 포함**.

**Snapshot 기반 백업.**

- **ZFS·Btrfs**: Snapshot 지원.
- **APFS (Mac)**: Time Machine 기반.
- **Windows File History**: Shadow Copy.
- **이점**: 과거 특정 시점 복구.

**Snapshot이 강력한 이유**—의도적 변경도 되돌림.

**박사의 백업 함정 7가지.**

- **"곧 할 예정"**: 미루다가 사고.
- **동기화를 백업으로**: Dropbox는 실수 삭제 동기화.
- **단일 매체**: 하드디스크만 백업.
- **미검증**: 복구 안 되는 백업.
- **암호화 안 함**: 도난 시 프라이버시 유출.
- **클라우드만**: Provider 폐업 위험.
- **백업 중단**: 몇 달 방치.
...
- **로컬 디스크**: FileVault (Mac)·BitLocker (Windows).
- **외장 드라이브**: VeraCrypt·FileVault.
- **클라우드**: Cryptomator·Boxcryptor·zero-knowledge 서비스.
- **이유**: 도난·분실 시 정보 유출.
...
- **절대 커밋 금지**: API key·password·개인정보.
- **.gitignore 예시**:
  ```
  *.env
  config/secrets.yml
  ...
  __pycache__/
  ```
- **Secrets Manager**: 환경변수·1Password·Vault.

**Git repo에 secret 들어가면 영구 기록**—즉시 rewrite history.
...
- **논문 제출 시**: 관련 모든 파일 ZIP·타임스탬프.
- **학위논문 완성**: 완전 아카이브 생성.
- **프로젝트 종료**: Read-only 저장.
- **졸업 시**: 전체 연구 아카이브.

**아카이브는 Active Backup과 별도**.

**학위논문 데이터 보관 의무.**

- **대부분 대학**: 학위논문 데이터 **5-10년 보관** 의무.
- **관련 규정**: Research Data Management Policy.
- **플랫폼**: Figshare·Zenodo·학교 Repository.
- **DOI 부여**: 영구 인용 가능.
...
- **별도 계정**: 각자의 프라이버시.
- **공동 계정**: 가족 사진·여행.
- **클라우드 Family Plan**: 비용 절감.

**부부 박사의 이점**—인프라 공유.

**재해 복구 계획 (DR Plan).**

- **Priority 1**: 박사 논문·핵심 데이터.
- **Priority 2**: 코드·분석.
- **Priority 3**: 문서·참고 자료.
- **Priority 4**: 부가 자료.
...
- **AI 모델 체크포인트**: GB~TB 단위.
- **Dataset 보관**: 재현성을 위한.
- **LLM Context**: 대화 기록 백업.
- **Cloud GPU 데이터**: 인스턴스 종료 시 손실 위험.
- **AI가 백업 자동화**: 스마트 스케줄.

**AI 시대 데이터 용량 폭증**—백업 전략도 업데이트.
...
**마지막 — 백업 없는 박사는 언제 무너질지 모르는 집이다.**

박사의 5-7년 데이터는 매 순간 손실 위험. 3-2-1 규칙·매체 비교·7가지 손실 시나리오·클라우드 옵션·학교 서비스·Git의 역할·대용량·자동화·검증·Versioning·Snapshot·7가지 함정·암호화·.gitignore·오프사이트·빈도·마일스톤·학위논문 보관·커플 공유·DR Plan·AI 시대·복구 심리·체크리스트·5년 진화 — 이 모든 것을 의식적으로 설계하면 박사가 **어떤 재해에도 복구 가능**하다. 백업은 박사의 보험.

> 백업은 박사의 침묵의 수호자. 3-2-1 규칙 (3개 복사본·2개 매체·1개 오프사이트). 매체 비교 (HDD·SSD·NAS·클라우드·LTO). 7가지 손실 시나리오 (고장·도난·실수·랜섬웨어·화재·서비스 중단·해킹). 클라우드 (Dropbox·Google·Backblaze). 학교 서비스 최우선. Git은 코드 백업. 대용량은 DVC·Rclone. 자동화 (Time Machine·cron·Dropbox)가 핵심. 분기 복구 검증. Versioning ≠ Backup. Snapshot 기반. 7가지 함정 (미루기·동기화로 착각·단일·미검증·암호 없음·클라우드만·중단). 암호화 (FileVault·BitLocker·Cryptomator). .gitignore 민감 정보. 오프사이트 위치 다양. 빈도는 손실 허용. 마일스톤 아카이브. 학위논문 5-10년 보관 의무. 커플 공유. DR Plan. 2024+ AI 모델·Dataset 백업. 복구의 Panic 방지. 10가지 체크리스트. 5년 진화. 백업은 박사의 보험.
